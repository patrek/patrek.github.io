...
I'm dreaming of the day when I would be invited to participate in a project from its inception, to be able to decide what would go in it and what would not.
...
It's like I've grown with the evolution of some technologies, learned from my mistakes.
...
And a QA would start everything.
    ...
    * https://mailpit.axllent.org/[mailpit] to see the emails sent by the application, it is a simple smtp server that can be used to see the emails sent by the application without having to send them to a real smtp server.
...
They are the first line of defence against bugs.
